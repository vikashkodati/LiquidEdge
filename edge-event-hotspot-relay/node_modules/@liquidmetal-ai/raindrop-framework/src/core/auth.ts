import { JWTVerifyResult, NoWay, decodeJwt } from '@liquidmetal-ai/noway';
 ...
 */
export const verifyIssuer = async (request: Request, env: Env): Promise<boolean> => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    if (token) {
      try {
        ...
        const jwt = await noWay.verifyJWTWithJWKS(
          token,
 ...
 */
export const requireAuthenticated = async (_request: Request, env: Env): Promise<boolean> => {
  return !!env.jwt;
};
